...
def excel(request):
    ...
    if request.method == 'POST':
        ...
        print("df1 :  ",df1)
        print("file read complete",df1.columns[0],' -- ',df2.columns)
        ...
        text_report = compare.report()
            # print(text_report)
        file_report_txt = settings.DATA_DIR/'Reprort.txt'
        ...
        print(type(records_present_in_df1_not_in_df2))
        ...
        #     df11 = df1.set_index(df1.columns[0])
        #     df12 = df2.set_index(df2.columns[0])
        #     df3 = pd.concat([df11,df2],sort=False)
        #     print("df3  :  ",df3)
        #     df3a = df3.stack().groupby(level=[0,1]).unique().unstack(1).copy()
        #     print("first excel data:    ",df3a)
        ...
        #     print(" Excel Data :   ",df3a)
        #     # print(" Excel List Data :   ",df3a.values.tolist())
        #     # df3a_list = df3a.values.tolist()
        #     # df3a = df3a.loc[df3a[df2.columns.to_list(
        #     # )].isnull().all(axis=1), df1.columns.to_list()]
        ...
        #         df1.to_excel(writer, sheet_name='SRC_data', index=False)
        #         df2.to_excel(writer, sheet_name='TARGET', index=False)
        #         df3a.to_excel(writer, sheet_name='DIFF Record', index=False)
        # else:
        #     print("columns are same")
        #     # data comparison business logic
        #     compare = datacompy.Compare(
...
#         print("file read complete",df1.columns[0])

#         # data comparion business logic
#         compare = datacompy.Compare(
#             df1, df2, join_columns=f'{df1.columns[0]}', df1_name='SRC_DATA',
#             df2_name='TGT_DATA')
#         text_report = compare.report()
...
def sql(request): 
    ...
    if request.method == 'POST' and 'htmlsubmitbutton2' in request.POST:
        ...
        if len(df1.columns)!=len(df2.columns):
            df01 = pd.read_excel('Excel1.xlsx')
            df02 = pd.read_excel('Excel2.xlsx')
            print('Columns are different')
            df11 = df01.set_index(df02.columns[0])
            df12 = df02.set_index(df01.columns[0])
            df3 = pd.concat([df11,df02],sort=False)
            df3a = df3.stack().groupby(level=[0,1]).unique().unstack(1).copy()

            df3a.loc[~df3a.index.isin(df02.index),'status'] = 'deleted' # if not in df2 index then deleted
            df3a.loc[~df3a.index.isin(df01.index),'status'] = 'new'     # if not in df1 index then new
            idx = df3.stack().groupby(level=[0,1]).nunique() # get modified cells.
            df3a.loc[idx.mask(idx <= 1).dropna().index.get_level_values(0),'status'] = 'modified'
            df3a['status'] = df3a['status'].fillna('same') # assume that anything not fufilled by above rules is the same.
            ...
            file_name = settings.DATA_DIR/'output.xlsx'
            with pd.ExcelWriter(file_name) as writer:
                df01.to_excel(writer, sheet_name='SRC_data', index=False)
                df02.to_excel(writer, sheet_name='TARGET', index=False)
                df3a.to_excel(
                    writer, sheet_name='DIFF Record', index=False)
            # df11 = df1.set_index(df1.columns[0])
            # df12 = df2.set_index(df2.columns[0])
